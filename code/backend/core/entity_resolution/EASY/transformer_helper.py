...
model_path = {
    "bert-base-cased": os.path.join(os.getcwd(), "core", "lm_models", "bert-base-cased")
}
...
class EmbeddingLoader(object):
    ...
    def __init__(self, model: str = "xlm-roberta-base", device=torch.device('cuda'), layer=1):
        ...
        if model in TR_Models:
            ...
            self.emb_model = model_class.from_pretrained(model_path[model], output_hidden_states=True)
            self.emb_model.eval()
            self.emb_model.to(self.device)
            self.tokenizer = tokenizer_class.from_pretrained(model_path[model], do_lower_case=False)
            ...
            if os.path.isdir(model):
                ...
                self.emb_model = AutoModel.from_pretrained(model_path[model], config=config)
                self.emb_model.eval()
                self.emb_model.to(self.device)
                self.tokenizer = AutoTokenizer.from_pretrained(model_path[model])
...
class BERT(object):
    ...
    def __init__(self, model='bert-base-cased', pool='max'):
        self.tokenizer = BertTokenizer.from_pretrained(model_path[model], do_lower_case=False)
        self.model = BertModel.from_pretrained(model_path[model], output_hidden_states=True)
        self.model.eval()
