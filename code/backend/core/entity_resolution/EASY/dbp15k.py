...
from core.entity_resolution.EASY.graph_utils import get_mask_with_sim
from core.entity_resolution.EASY.text_sim import *
from core.entity_resolution.EASY.transformer_helper import BERT, EmbeddingLoader
...
class DBP15k(InMemoryDataset):
    ...
    def __init__(self, pair, task_path: str, device='cuda',
                 transform=None, pre_transform=None,_eval:bool=True,):
        assert pair in DBP15K_PAIRS
        self.pair = pair
        self.device = device
        self._eval = _eval
        self.task_path = task_path
        root = os.path.join(self.task_path, "dataset")
    ...
    @torch.no_grad()
    def process(self):
        lang_sr, lang_tg = str(self.pair).split("_")
        g1_path = osp.join(self.root, f'triples_{lang_sr}')
        g2_path = osp.join(self.root, f'triples_{lang_tg}')
        x1_path = osp.join(self.root, f'id_features_{lang_sr}')
        x2_path = osp.join(self.root, f'id_features_{lang_tg}')

        attrgnn_name2id_path = [osp.join(self.root, f"entity2id_{lang}.txt")
                                for lang in [lang_sr, lang_tg]]
        ...
        name_paths = [osp.join(self.root, f"ent_ids_{lang}") for lang in [lang_sr, lang_tg]]
        ...
        print(self.root)
        saveobj(n2i_dataset, osp.join(self.root, 'ent_name2id'))
        # translates = self.read_translate(n2i_dataset)
        # name_words1, name_words2 = translates
        total_y = self.process_y_n2id(osp.join(self.root, "entity_seeds.txt"),
