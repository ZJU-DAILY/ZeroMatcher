...
class BERT(object):
    ...
    def __init__(self, model='bert-base-cased', pool='max'):
        self.tokenizer = BertTokenizer.from_pretrained(os.path.join(os.getcwd(),"core","lm_models",model),local_files_only=True,do_lower_case=False)
        self.model = BertModel.from_pretrained(os.path.join(os.getcwd(),"core","lm_models",model),local_files_only=True, output_hidden_states=True)
        self.model.eval()
