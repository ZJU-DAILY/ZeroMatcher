...
from core.entity_resolution.LargeEA import text_utils
from core.entity_resolution.LargeEA.partition import *
# from core.entity_resolution.LargeEA.models import ModelWrapper
...
class AlignmentBatch:
    def __init__(self, triple1, triple2, src_nodes, trg_nodes, train_pairs, test_pairs, backbone,
        ...
        logger.info("Batch info: ", '\n\t'.join(map(lambda x, y: '='.join(map(str, [x, y])),
                                                    ...
                                                    map(len,
                                                        [triple1, triple2, src_nodes, trg_nodes, train_pairs,
                                                         test_pairs]))))
        try:
            ...
            print(np.array(self.train_ill)[:5])
            self.model.update_trainset(np.array(self.train_ill).T)
            logger.info('build_batch_info')
...
def batch_sampler(data: EAData, logger: loguru.Logger, src_split=30, trg_split=100, top_k_corr=5, which=0,
                  share_triples=True, backbone='RREA', random=False):
    ...
    logger.info('graph_partition')
    print(data.triples[which][:5])
    print(data.triples[1 - which][:5])
    ...
    for k in triple1_batch.keys():
        print(triple1_batch[k][:5])
    logger.info('place_triples')
    ...
    logger.info('get_corr_batch')
    # corr_ind = corr_ind.numpy()
    logger.info(f'partition complete, time={time.time() - time_now}')
