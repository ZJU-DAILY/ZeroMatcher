...
from core.entity_resolution.LargeEA.utils import *
import os
import os.path as osp
from random import shuffle
import codecs
from core.entity_resolution.LargeEA.dto import *


class EAData:
    def __init__(self, data_path, triple1_path, triple2_path, ent_links_path,
        ...
        self.rels = [rel1, rel2]
        self.ents = [ent1, ent2]
        n2i_dataset = (ent1, ent2)
        saveobj(n2i_dataset, osp.join(data_path, 'ent_name2id'))
...
class OpenEAData(EAData):
    def __init__(self, path, pair: str, **kwargs):
        lang_sr, lang_tg = pair.split("_")
        super().__init__(path,
                         osp.join(path, f'triples_{lang_sr}'),
                         osp.join(path, f'triples_{lang_tg}'),
                         osp.join(path, 'ent_links'), **kwargs)


class LargeScaleEAData(EAData):
    def __init__(self, path, pair: str, strict=False, shuffle_pairs=False, train_ratio=0.2, **kwargs):
        ...
        super().__init__(path,
                         osp.join(path, f'triples_{lang_sr}'),
                         osp.join(path, f'triples_{lang_tg}'),
                         osp.join(path, 'ent_links'), shuffle_pairs, train_ratio)
