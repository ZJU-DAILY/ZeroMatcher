...
from core.entity_resolution.EASY.bootstrap import *
from core.utils import getTaskLogger
from core.entity_resolution.EASY.eval import *
from core.entity_resolution.EASY.graph_utils import *
# from models import ModelWrapper
from core.entity_resolution.gnn_models.wrapper import ModelWrapper
from core.entity_resolution.EASY.utils import *
import torch
import torch.nn as nn


class FinalFantasy(nn.Module):
    def __init__(self, logger: loguru.Logger, dataset, task_path, _pair: str, dim=200, name="",
                 device='cuda', srp=True, model='rrea',
                 fuse_semantic=True, fuse_lev_dist=True, _eval: bool = True):
        ...
        # ZXC tuple[torch,torch]
        self.current_gnn_embedding = None

    def save_gnn_emb(self, save_path: str):
        np.save(os.path.join(save_path, "embA.npy"), self.current_gnn_embedding[0].numpy())
        np.save(os.path.join(save_path, "embB.npy"), self.current_gnn_embedding[1].numpy())

    def load_gnn_emb(self, save_path: str):
        self.current_gnn_embedding = (
            torch.from_numpy(np.load(os.path.join(save_path, "embA.npy"))),
            torch.from_numpy(np.load(os.path.join(save_path, "embB.npy"))))
    ...
    def get_gnn_embed(self) -> List[Tensor]:
        if self.current_gnn_embedding is not None:
            embeds = apply(lambda x: x.to(self.device), *self.current_gnn_embedding)
        else:
    ...
    def train_myself(self, num_it=20, epoch=20, refine_begin=1, iter_type='ours', only_gnn=False):
        ...
        for curr_it in range(num_it):
            self.logger.info("Iteration {0}".format(curr_it))
            self.logger.info("refinement")
            self.refinement(curr_it, refine_begin, iter_type)
            self.logger.info("train1step")
            self.model.train1step(epoch)
            if self._eval:
                self.logger.info("eval")
                self.eval_myself(only_gnn)
            if curr_it == num_it - 1:
                self.current_gnn_embedding = self.model.get_curr_embeddings()
        # self.save_align_pair()
    ...
    def new_log(self, **kwargs):
        ...
        # name = self.name
        # log_path = os.path.join(self.task_path, "log")
        # if not os.path.exists(log_path):
        #     os.mkdir(log_path)
        # np.save(f"{log_path}" + f"/{name}_{it}", self.get_current_perm().cpu().numpy())
    ...
    def eval_myself(self, only_gnn=False, eval_tasks=None):
        ...
        for task in eval_tasks:
            if hasattr(self.dataset, task):
                self.logger.info(f'Eval task is {task}')
    ...
    @torch.no_grad()
    def save_align_pair(self):
        ground_truth = getattr(self.dataset, "total_y")
        # 对应ent_idx的下标
        print(ground_truth)
        perm = self.get_current_perm()
        if perm.is_sparse:
            perm = perm.to_dense()
        match_sim0, match_id0, sim_matrix0 = get_topk_sim(perm)
        # 这里match_id0是表1每个实体前10个相似的表2实体id，拿第一个作为对齐实体对
        id = 0
        pairs = []
        for item in match_id0:
            pairs.append([int(id), int(item[0].item())])
            id += 1
        pairs = np.array(pairs)
        save_path = os.path.join(self.task_path, "dataset", "result.txt")
