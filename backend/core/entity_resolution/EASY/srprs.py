...
class SRPRS(InMemoryDataset):
    def __init__(self, pair, task_path: str, device="cuda",
                 ...
                 mean_pool=False,_eval:bool=True,):
        self.pair = pair
        self.lang_sr, self.lang_tg = str(self.pair).split("_")
        self.SRPRS_REL1 = f'/triples_{self.lang_sr}'
        self.SRPRS_REL2 = f'/triples_{self.lang_tg}'
    ...
    def raw_file_names(self):
        return [ATTR1, ATTR2, ENT_LINKS, self.SRPRS_REL1, self.SRPRS_REL2]
    ...
    def process_link(links_pos, ent1, ent2):
        ...
        with open(links_pos, "r") as f:
            for line in f.readlines():
                now = line.strip().split('\t')
                # ZXC
                if now[0] not in ent1.keys() or not now[1] in ent2.keys():
                    continue
    ...
    @torch.no_grad()
    def process(self):
        # root = osp.join(self.raw_dir)
        root=self.root
        bert_model = 'bert-base-multilingual-cased' if self.mul_bert else 'bert-base-cased'

        self.graph1, self.rel1, self.ent1 = self.process_one_graph(root + self.SRPRS_REL1)
        self.graph2, self.rel2, self.ent2 = self.process_one_graph(root + self.SRPRS_REL2)
        ...
        graph = Data(x1=x1, x2=x2, sim_x2y=sim_x2y, sim_y2x=sim_y2x, x2y_mask=x2y_mask, y2x_mask=y2x_mask,
                     edge_index1=ei1, edge_index2=ei2, rel1=et1, rel2=et2, total_y=total_y, **sims)
