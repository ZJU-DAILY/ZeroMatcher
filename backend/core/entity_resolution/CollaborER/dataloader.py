...
from core.entity_resolution.CollaborER.utils import read_entity, get_tokenizer


class TrainDataset(Dataset):
    def __init__(self, path, pair: str, lm='bert', max_length=256, skip=True, add_token=True):
        self.path = path
        self.tokenizer = get_tokenizer(lm)
        self.lang_sr, self.lang_tg = pair.split("_")
        tableA = os.path.join(path, f'table_{self.lang_sr}.csv')
        tableB = os.path.join(path, f'table_{self.lang_tg}.csv')
...
class TestDataset(Dataset):
    def __init__(self, path, pair: str, test=True, lm='bert', max_length=512, skip=False, add_token=True):
        self.path = path
        self.tokenizer = get_tokenizer(lm)
        self.lang_sr, self.lang_tg = pair.split("_")
        tableA = os.path.join(path, f'table_{self.lang_sr}.csv')
